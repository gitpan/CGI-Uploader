...
To process the update form, first delete any uploads that the user has
...
=head2 Deleting An Upload

If you need another way to delete an upload besides using C<delete_checked_uploads> above,
you can use the following:

 my $field_name = $u->delete_upload('photo', $photo_id );

This does not current remove the the related thumbnails as well. 
