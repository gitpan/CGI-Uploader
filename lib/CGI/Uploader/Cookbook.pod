...
mananging file uploads as a professional website developer for
Summersault, LLC (L<http://www.summersault.com/>). Over that time I noticed patterns 
that were re-usable from project to project. I went through serveral versions
...
I have strived to make CGI::Uploader to work within a variety of system designs.
...
Finally, C<transform_meta()> is a basic function which tranforms a hashref of data from the database
...
creating the thunbmails. Support for C<GD> is in progress. C<GD> supports many
...
to store, update, delete and view the uploads attached to sme database entity. Those
...
example application, please see the C<examples> directory of the distribition.
...
This hashref can often be passed directly to a templating sytem such as
L<HTML::Template|HTML::Template> for display.

You may be wondering about the query strings on the URLS. These are random
numbers to defeat browser image caching, which is very useful on upgrade
...
=head2 Recipe Idea: Put an existing directory of photos online 

You have an existing directory full of JPEGS that you want to put online
as a photo gallery, with medium and small versions created of all the images.
C<CGI::Uploader> is versitale enough to help in thi situation as well.
...
To display all the smallest thumbnails on a single page....
XXX Finish me. (Hmmm, with the foreign keys, we've lost
a way to tell for sure which thumnbail is which. ) 





=head2 Recipe Idea: Further automation

C<CGI::Uploader> Allows you to build your specification on the fly,
when the exact fields you may need to process may vary. To do this,
the keys defined in the C<spec> can regular expressionss. The simplest one wuld
automatically process all file upload fields delivered through the form.

 spec => { qr/.*/ => [] },

If you generate thumbnails this case, you'll need a way to give the thumbnails
for each fields unique names. To do this, you can define the name as a code
reference. It will receive the name of the parent file field as it's first argument:
...
For a matching field named C<photo_2>, the resulting thumbnail would be
named C<photo_2_thumb>.

=head2 Recipe Idea: Handling anonymous image uploads

The recipe above required having a column name in another table for each upload
that was stored. It's also possible with CGI::Uploader to have many "anynonmous" uploads
